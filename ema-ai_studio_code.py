...
# TV Chart Colors
COLORS = {
    'Nifty 50': '#2962FF',     # Bright Blue
    'Crude Oil': '#E91E63',    # Pink
    'Gold': '#FF9800',         # Orange
...
def load_data(period):
    ...
    for name, ticker in TICKERS.items():
        try:
            ...
            if not hist.empty and 'Close' in hist.columns:
                s = hist['Close'].copy()
                s.name = name
                # Force every series index to strict UTC Datetime BEFORE merging
    ...
    # Remove duplicates, sort chronologically, and fill missing data safely
...
# 1. Normalize data to base 100 for comparison
normalized_df = (df / df.iloc[0]) * 100

# 2. Calculate EMA 21 on the normalized data
ema_df = normalized_df.ewm(span=21, adjust=False).mean()

# 3. Format Data for TradingView JS (Needs Unix Timestamps in seconds)
tv_data = {}
tv_ema_data = {}

for col in normalized_df.columns:
    # Build list of dictionaries for prices
    price_series =[{"time": int(ts.timestamp()), "value": float(val)} 
                    for ts, val in normalized_df[col].items() if pd.notna(val)]
    tv_data[col] = price_series
    
    # Build list of dictionaries for EMA
    ema_series =[{"time": int(ts.timestamp()), "value": float(val)} 
                  for ts, val in ema_df[col].items() if pd.notna(val)]
    tv_ema_data[col] = ema_series

# Serialize to JSON to pass into Javascript
chart_data_json = json.dumps(tv_data)
ema_data_json = json.dumps(tv_ema_data)
colors_json = json.dumps(COLORS)
show_ema_js = "true" if show_ema else "false"

# Generate HTML/JS for TradingView Lightweight Charts
html_string = f"""
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        ...
        .legend {{ position: absolute; top: 10px; left: 10px; z-index: 10; font-size: 14px; background: rgba(19, 23, 34, 0.8); padding: 5px; border-radius: 5px;}}
...
# --- CORRELATION CHART SECTION (Plotly is still best for Heatmaps) ---
...
if corr_type == "Returns Correlation (Recommended)":
    # .fillna(0) prevents Plotly from crashing if market price is flatlined during off-hours
...
# --- RAW DATA SECTION ---
st.subheader("3. Latest Live Market Prices (UTC Time)")
